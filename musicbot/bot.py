...
class MusicBot(discord.Client):
    def __init__(self, config_file=ConfigDefaults.options_file, perms_file=PermissionsDefaults.perms_file):
        ...
        if not self.autoplaylist:
            print("Warning: Автоплейлист пустой, выключаюсь.")
    ...
    async def _auto_summon(self):
        ...
        if owner:
            self.safe_print("Владелец найден в \"%s\", пытаюсь подключиться..." % owner.voice_channel.name)
            # TODO: Effort
    ...
    async def _check_ignore_non_voice(self, msg):
        ...
        if not vc or vc == msg.author.voice_channel:
            ...
            raise exceptions.PermissionsError(
                "Смертный, войди в мои владения и, лишь тогда, ты сможешь обратиться ко мне! (%s)" % vc.name, expire_in=30)
        ...
        if server.id not in self.players:
            if not create:
                raise exceptions.CommandError(
                    'Дракон покинул логово.  '
                    'Попробуй украсть его золото, (%ssummon) чтобы призвать Дракона в его логово.' % self.config.command_prefix)
    ...
    async def on_player_play(self, player, entry):
        ...
        if channel and author:
            ...
            if self.config.now_playing_mentions:
                newmsg = '%s - твоя песня **%s** сейчас играет в %s!' % (
    ...
    async def update_now_playing(self, entry=None, is_paused=False):
        ...
        if self.user.bot:
            ...
            if activeplayers > 1:
                game = discord.Game(name="музыку на %s серверах" % activeplayers)
                entry = None
    ...
    async def cmd_blacklist(self, message, user_mentions, option, something):
        ...
        Добавляет, или удаляет пользователей из ЧС.
        """

        if not user_mentions:
            raise exceptions.CommandError("Похоже, никто не сгорел.", expire_in=20)
        ...
        for user in user_mentions.copy():
            if user.id == self.config.owner_id:
                print("[Commands:Blacklist] Владельца нельзя сжечь.")
        ...
        if option in ['+', 'add']:
            ...
            return Response(
                '%s смертный был сожжен.' % (len(self.blacklist) - old_len),
                reply=True, delete_after=10
            )

        else:
            if self.blacklist.isdisjoint(user.id for user in user_mentions):
                return Response('этого смертного я не сжигал.', reply=True, delete_after=10)
                ...
                return Response(
                    '%s смертный восстал из пепла.' % (old_len - len(self.blacklist)),
                    reply=True, delete_after=10
                )
    ...
    async def cmd_id(self, author, user_mentions):
        ...
        Говорит пользователю его ID или ID другого пользователя.
        """
        if not user_mentions:
    ...
    async def cmd_joinserver(self, message, server_link=None):
       ...
       Просит бота присоединиться к серверу.
        """

        if self.user.bot:
            url = await self.generate_invite_link()
            return Response(
                "Боты не могут пользоваться ссылками!  Тык сюда и пригласи меня: \n{}".format(url),
    ...
    async def cmd_play(self, player, channel, author, permissions, leftover_args, song_url):
        ...
        if permissions.max_songs and player.playlist.count_for_user(author) >= permissions.max_songs:
            raise exceptions.PermissionsError(
                "Ты достиг лимита песен, смертный, проси прощения! (%s)" % permissions.max_songs, expire_in=30
            )
        ...
        if not info:
            raise exceptions.CommandError("Это видео нельзя проиграть.", expire_in=30)
        ...
        if 'entries' in info:
            ...
            if permissions.max_playlist_length and num_songs > permissions.max_playlist_length:
                raise exceptions.PermissionsError(
                    "Ты разгневал меня своим запросом! (%s > %s)" % (num_songs, permissions.max_playlist_length),
                    expire_in=30
                )
            ...
            if permissions.max_songs and player.playlist.count_for_user(author) + num_songs > permissions.max_songs:
                raise exceptions.PermissionsError(
                    "Ты разгневал меня своим запросом! Попробуй в следующий раз, может, удача будет благоволить тебя и у тебя выйдет. (%s + %s > %s)" % (
            ...
            procmesg = await self.safe_send_message(
                channel,
                'Получаю информацию о плейлисте содержащем {} песен{}'.format(
                    num_songs,
            ...
            if permissions.max_song_length:
                ...
                if drop_count:
                    print("Съедено %s песен" % drop_count)

            print("Переварено {} песен за {} секунд {:.2f}s/song, {:+.2g}/song from expected ({}s)".format(
                listlen,
                self._fixg(ttime),
                ttime / listlen,
            ...
            if not listlen - drop_count:
                raise exceptions.CommandError(
                    "Не было добавлено песен, они слишком длинные, кого ты пытаешься обмануть? (%ss)" % permissions.max_song_length,
                    expire_in=30
                )

            reply_text = "Песни **%s** поставлены в очередь. Позиция в очереди: %s"
            btext = str(listlen - drop_count)

        else:
            if permissions.max_song_length and info.get('duration', 0) > permissions.max_song_length:
                raise exceptions.PermissionsError(
                    "Ты слишком многого желаешь, смертный (%s > %s)" % (info['duration'], permissions.max_song_length),
                    expire_in=30
                )
            ...
            reply_text = "Твоя просьба будет исполнена, смертный **%s**. Позиция в очереди: %s"
            btext = entry.title
    ...
    async def _cmd_play_playlist_async(self, player, channel, author, permissions, playlist_url, extractor_type):
        ...
        if not info:
            raise exceptions.CommandError("Этот плейлист не может быть проигран.")

        num_songs = sum(1 for _ in info['entries'])
        t0 = time.time()

        busymsg = await self.safe_send_message(
            channel, "Переваривание %s песен..." % num_songs)  # TODO: From playlist_title
        await self.send_typing(channel)
        ...
        if not songs_added:
            basetext = "Не было добавлено ни одной песни, твой запрос превышает все границы, смертный! (%ss)" % permissions.max_song_length
            if skipped:
                basetext += "\nAdditionally, the current song was skipped for being too long."

            raise exceptions.CommandError(basetext, expire_in=30)

        return Response("Песни {} добавлены в очередь за {} секунд".format(
    ...
    async def cmd_search(self, player, channel, author, permissions, leftover_args):
        ...
        if permissions.max_songs and player.playlist.count_for_user(author) > permissions.max_songs:
            raise exceptions.PermissionsError(
                "Моё терпение уже на грани! (%s)" % permissions.max_songs,
                expire_in=30
            )
        ...
        try:
            ...
            raise exceptions.CommandError("Будь осторожен со своими желаниями.", expire_in=30)
        ...
        if leftover_args[0].isdigit():
            ...
            if items_requested > max_items:
                raise exceptions.CommandError("Ты не можешь просить более %s видео" % max_items)
        ...
        search_msg = await self.send_message(channel, "Ищу видео...")
        await self.send_typing(channel)
        ...
        for e in info['entries']:
            ...
            confirm_message = await self.safe_send_message(channel, "Это оно? Введи `y`, `n` или `exit`")
            response_message = await self.wait_for_message(30, author=author, channel=channel, check=check)

            if not response_message:
                await self.safe_delete_message(result_message)
                await self.safe_delete_message(confirm_message)
                return Response("В следующий раз хорошо подумай, смертный!.", delete_after=30)
            ...
            if response_message.content.lower().startswith('y'):
                ...
                return Response("Твоя просьба выполнима, смертный.", delete_after=30)
            else:
                await self.safe_delete_message(result_message)
                await self.safe_delete_message(confirm_message)
                await self.safe_delete_message(response_message)

        return Response("Ну хорошо :frowning:", delete_after=30)
    ...
    async def cmd_np(self, player, channel, server, message):
        ...
        if player.current_entry:
            ...
            if player.current_entry.meta.get('channel', False) and player.current_entry.meta.get('author', False):
                np_text = "Сейчас играет: **%s** добавленная **%s** %s\n" % (
            ...
            return Response(
                'В очереди нет просьб! Попробуй, попроси что-нибдь с помощью {}play.'.format(self.config.command_prefix),
                delete_after=30
            )
    ...
    async def cmd_summon(self, channel, author, voice_channel):
        ...
        if not author.voice_channel:
            raise exceptions.CommandError('Я не вижу места для нового логова!')
    ...
    async def cmd_pause(self, player):
        ...
        if player.is_playing:
            ...
            raise exceptions.CommandError('Просьбы не выполняются.', expire_in=30)
    ...
    async def cmd_resume(self, player):
        ...
        if player.is_paused:
            ...
            raise exceptions.CommandError('Просьбы и так выполняются.', expire_in=30)
    ...
    async def cmd_skip(self, player, channel, author, message, permissions, voice_channel):
        ...
        Пропускает песню по достижению кол-ва голосов или по приказу владельца.
        """

        if player.is_stopped:
            raise exceptions.CommandError("Не могу пропустить! Никаких просьб нет!", expire_in=20)

        if not player.current_entry:
            if player.playlist.peek():
                if player.playlist.peek()._is_downloading:
                    # print(player.playlist.peek()._waiting_futures[0].__dict__)
                    return Response("Следующая песня (%s) загружается." % player.playlist.peek().title)

                elif player.playlist.peek().is_downloaded:
                    print("Следующая песня будет играть скоро.")
                else:
                    print("Произошло что-то странное.  "
                          "Вы можете перезапустить бота, если он не начал работать.")
            else:
                print("Произошло что-то странное.  "
                      "Вы можете перезапустить бота, если он не начал работать.")
        ...
        if skips_remaining <= 0:
            player.skip()  # check autopause stuff here
            return Response(
                'твой голос за пропуск **{}** был принят.'
                '\n голосование за пропуск прошло.{}'.format(
                    player.current_entry.title,
                    ' Следующая песня на подходе!' if player.playlist.peek() else ''
                ),
                reply=True,
                delete_after=20
            )
            ...
            return Response(
                'твой голос за пропуск **{}** был принят.'
                '\n**{}** нужно ещё {} голосов для пропуска.'.format(
                    player.current_entry.title,
                    skips_remaining,
    ...
    async def cmd_volume(self, message, player, new_volume=None):
        ...
        try:
            ...
            raise exceptions.CommandError('{} не допустимая цифра'.format(new_volume), expire_in=20)
        ...
        if 0 < new_volume <= 100:
            ...
            return Response('громкость изменена с %d на %d' % (old_volume, new_volume), reply=True, delete_after=20)

        else:
            if relative:
                raise exceptions.CommandError(
                    'Неправильное изменение громкости: {}{:+} -> {}%.  Изменение громкости происходит между {} и {:+}.'.format(
                        old_volume, vol_change, old_volume + vol_change, 1 - old_volume, 100 - old_volume), expire_in=20)
            else:
                raise exceptions.CommandError(
                    'Неправильное изменение громкости: {}%. Громкость меняется между 1 и 100.'.format(new_volume), expire_in=20)
    ...
    async def cmd_queue(self, channel, player):
        ...
        lines = []
        unlisted = 0
        andmoretext = '* ... и ещё %s*' % ('x' * len(player.playlist.entries))
        ...
        if player.current_entry:
            ...
            if player.current_entry.meta.get('channel', False) and player.current_entry.meta.get('author', False):
                lines.append("Сейчас играет: **%s** добавленная **%s** %s\n" % (
                    player.current_entry.title, player.current_entry.meta['author'].name, prog_str))
            else:
        ...
        for i, item in enumerate(player.playlist, 1):
            if item.meta.get('channel', False) and item.meta.get('author', False):
                nextline = '`{}.` **{}** добавленная **{}**'.format(i, item.title, item.meta['author'].name).strip()
            else:
        ...
        if not lines:
            lines.append(
                'В плейлисте нет песен! Добавьте что-нибудь в плейлист с помощью {}play.'.format(self.config.command_prefix))
    ...
    async def cmd_clean(self, message, channel, server, author, search_range=50):
        ...
        if self.user.bot:
            if channel.permissions_for(server.me).manage_messages:
                ...
                return Response('Убрано {} сообщений{}.'.format(len(deleted), 's' * bool(deleted)), delete_after=15)
        ...
        return Response('Убрано {} сообщений{}.'.format(deleted, 's' * bool(deleted)), delete_after=15)
    ...
    async def on_voice_state_update(self, before, after):
        ...
        if sum(1 for m in my_voice_channel.voice_members if m != after.server.me):
            if auto_paused and player.is_paused:
                print("[config:autopause] Снимаю с паузы")
                self.server_specific_data[after.server]['auto_paused'] = False
                player.resume()
        else:
            if not auto_paused and player.is_playing:
                print("[config:autopause] Ставлю на паузу")
                self.server_specific_data[after.server]['auto_paused'] = True
                player.pause()

    async def on_server_update(self, before:discord.Server, after:discord.Server):
        if before.region != after.region:
            self.safe_print("[Servers] \"%s\" сменил регионы: %s -> %s" % (after.name, before.region, after.region))
